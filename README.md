...
直接在代码中使用 `SimulatorEngine` 时，先通过 `src.application.assign_initial_lp_owner`
处理初始 LP 份额归属；常规配置运行路径使用 `SimulationRunner` 即可，Runner 会自动完成这一步。
...
| `tests/test_web_support.py` | Web 层数据转换、配置保存和运行清理 |
