...
class PDFReportGenerator:
    ...
    def _add_chart_section(self, plot_dir: str) -> None:
        ...
        chart_catalog = {
            "pool_spot_price.png": ("Pool Spot Price", "Spot price movement across processed events."),
            "pool_reserves.png": ("Pool Reserves", "Token reserve changes caused by swaps and liquidity events."),
            "swap_slippage.png": ("Swap Slippage", "Execution slippage observed for swap events."),
            "cumulative_fees.png": ("Cumulative Fees", "Fee accumulation over the simulation timeline."),
            "impermanent_loss.png": ("Impermanent Loss", "LP impermanent loss across the simulation."),
            "user_total_pnl.png": ("User PnL", "Final user-level profit and loss in Token Y terms."),
            "candlestick.png": ("Pool Price Candlestick", "OHLC-style view of pool price movement."),
            "slippage_volume.png": ("Slippage vs Volume", "Relationship between trade size and execution slippage."),
            "multi_scenario_comparison.png": ("Multi-Scenario Comparison", "Cross-scenario comparison of key simulation metrics."),
        }
        ordered_names = list(chart_catalog)
        available = {path.name: path for path in plot_dir_path.glob("*.png")} if plot_dir_path.exists() else {}

        existing: list[tuple[str, Path, str]] = []
        for filename in ordered_names:
            path = available.pop(filename, None)
            if path is None:
                continue
            title, caption = chart_catalog[filename]
            existing.append((title, path, caption))

        for filename, path in sorted(available.items()):
            title = filename.removesuffix(".png").replace("_", " ").title()
            existing.append((title, path, "Additional diagnostic chart generated by the simulator."))
